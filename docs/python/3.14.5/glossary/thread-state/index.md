...
The information used by the [CPython](../CPython/index.md) runtime to run in an OS thread.
...
[attached](../attached-thread-state/index.md) at once.

An [attached thread state](../attached-thread-state/index.md) is required to call most
...
thread, but only one can be [attached](../attached-thread-state/index.md) in
that thread at any given moment.

See [Thread State and the Global Interpreter Lock](https://docs.python.org/3.14/c-api/threads.html#threads) for more
information.
