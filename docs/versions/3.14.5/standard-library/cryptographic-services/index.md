...
The standard library provides **hash functions**, **message authentication**, and **cryptographically secure randomness** for application security. Use [`hashlib`](hashlib-secure-hashes-and-message-digests/index.md) for SHA-2/SHA-3/BLAKE2 digests; [`hmac`](hmac-keyed-hashing-for-message-authentication/index.md) for keyed MACs over those hashes; [`secrets`](secrets-generate-secure-random-numbers-for-managing-secrets/index.md) for tokens and passwords (never use [`random`](../numeric-and-mathematical-modules/random-generate-pseudo-random-numbers-with-various-distributions/index.md) for secrets). Algorithm availability depends on the build (OpenSSL, FIPS mode). Full API reference remains on [docs.python.org](https://docs.python.org/3/library/crypto.html).

Related material: [`ssl`](../internet-protocols-and-support/ssl-tls-wrapper-for-socket-objects/index.md) for TLS, [`hashlib.file_digest`](hashlib-secure-hashes-and-message-digests/index.md) for streaming file hashes, and [`base64`](../binary-data-services/base64-base16-base32-and-base64-data-encodings/index.md) for encoding digests.
